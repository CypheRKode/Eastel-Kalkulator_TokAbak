...
function getActiveLevels(base) {
  if (base >= 3500) return 13;
  if (base >= 500) return 8;
  return 3;
}

function getRank(base) {
  if (base >= 3500) return "Senior Ambassador";
  if (base >= 500) return "Ambassador";
  return "Elite Member";
}

function getProgress(base) {
  if (base >= 3500) return 100;
  if (base >= 500) {
    const progressToSenior = ((base - 500) / (3500 - 500)) * 100;
    return Math.max(0, Math.min(100, progressToSenior));
  }

  const progressToAmbassador = (base / 500) * 100;
  return Math.max(0, Math.min(100, progressToAmbassador));
}

function getUnlockMessage(base, rank) {
  if (rank === "Senior Ambassador") {
    return "L9 hingga L13 dibuka automatik.";
  }

  if (rank === "Ambassador") {
    const need = Math.max(0, 3500 - base);
    return `Perlu lagi ${formatMoney(need)} untuk buka Senior Ambassador.`;
  }

  const need = Math.max(0, 500 - base);
  return `Perlu lagi ${formatMoney(need)} untuk buka Ambassador.`;
}
